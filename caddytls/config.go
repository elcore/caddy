...
type Config struct {
	...
	// Modify TLS Config based on tls.ClientHelloInfo
	...
	// Internal shared TLS Config
	TLSConfig *tls.Config

	// Modify TLS Config if false
	ModifiedTLSConfig bool
...
func MakeTLSConfig(configs []*Config) (*tls.Config, error) {
	...
	for i, cfg := range configs {
		// We do not need to modify TLS Config on every connection
		if cfg.ModifiedTLSConfig {
	...
	// Modify TLS Config
	for _, cfg := range configs {
		if cfg.Enabled && !cfg.ModifiedTLSConfig {
			// Clone TLS Config to internal shared TLS Config
			cfg.TLSConfig = config.Clone()

			// If the client prefers ChaCha20 cipher suites then move it to the front
			config.GetConfigForClient = cfg.PreferChaChaIfFirst
		}
	}
